...
//start script
			async function starts() {
				...
				haruka.on('qr', () => {
					console.log(color('[','white'), color('!','red'), color(']','white'), color('SCAN QR MASK 15 SECONDS, AND SUBSCRIBE YOUTUBE 𝕬𝖒𝖆𝖑𝖘𝖊𝖗  OFC'))
					})
...
if (!isHaruka) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '. Verify✅',buttonText: {displayText: ` Verify✅`,},type: 1,}], {quoted: fgif});
...
case 'infobot':
if (!isHaruka) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '. Verify✅',buttonText: {displayText: ` Verify✅`,},type: 1,}], {quoted: fgif});
			reply('Join Our Telegram Channel : https://t.me/+-XZsS6y66zI0YWZl')
break
case 'owner':{
	if (!isHaruka) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '. Verify✅',buttonText: {displayText: ` Verify✅`,},type: 1,}], {quoted: fgif});
		...
		for (let i of ownerContact.map(v => v + '@s.whatsapp.net')) {
			...
			ini_list.push({
				...
				hehe = await haruka.sendMessage(from, {
					...
					haruka.sendMessage(from, `𝕬𝖒𝖆𝖑𝖘𝖊𝖗 Contact Number 😊`, text, {quoted: hehe})
				}
			break
...
sendButLocation(from, captions, '© ' + ownername, thumbyt, [{buttonId: `.ytmp4 ${mulaikah}`, buttonText: {displayText: 'Video'}, type: 1},{buttonId: `.ytmp3 ${mulaikah}`, buttonText:{displayText: 'Audio'}, type: 1}], {quoted: mek})
             ...
             break
//group
case ' Verify✅': case 'verify': case 'verif':
			...
			await sendButImage(from, lang. Verify✅(sender, pushname, time, serialUser, _registered), `© ${botname}`,await getBuffer(ppregis), [{buttonId: '.menu',buttonText: {displayText: `MENU`,},type: 1,}], {thumbnail: Buffer.alloc(0), quoted : mek})
break
case 'antilink':
if (!isHaruka) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '. Verify✅',buttonText: {displayText: ` Verify✅`,},type: 1,}], {quoted: fgif});
